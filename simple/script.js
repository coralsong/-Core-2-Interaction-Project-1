// variable named index is created, 0 means the first sentence in the array. 
let index = 0;

// array, aka list. 0 = first sentence, 1 = second sentence, etc.
  const messages = [
    ...
    "“No,” he replied. Then, as if confiding a secret, he lowered his voice.",
    "“I acquired the book in a town out on the plain in exchange for a handful of rupees and a Bible.",
    "Its owner did not know how to read.",
    ...
    "I lay my left hand on the cover and, trying to put my thumb on the flyleaf, I opened the book.",
    "It was useless.",
    ...
    "Perhaps to suggest that the terms of an infinite series admit any number.”",
    "Then, as if he were thinking aloud, he said, “If space is infinite, we may be at any point in space.",
    "If time is infinite, we may be at any point in time.”",
    "His speculations irritated me.",
    "“You are religious, no doubt?” I asked him.",
    "“Yes, I’m a Presbyterian. My conscience is clear.",
    ...
    "I thought of keeping the Book of Sand in the space left on the shelf by the Wiclif,",
    "but in the end I decided to hide it behind the volumes of a broken set of “The Thousand and One Nights.”",
    ...
    "I had only a few friends left;",
    "I now stopped seeing even them.",
    ...
    "One day I went there and,",
    "slipping past a member of the staff and trying not to notice at what height or distance from the door,",
    "I lost the Book of Sand on one of the basement’s musty shelves."
  ];

  // calls "type" from html and stores it in variable textEl.
const textEl = document.getElementById("type");
const inputEl = document.getElementById("searchInput");

// whenever function is called, text is typed.
function showMessage(text) {
  // clears previous text out before new text shows.
  textEl.textContent = "";
  // i tracks which letter is being typed. 0 = first letter.
  let i = 0;

  // sets a timer for function called. 
  const interval = setInterval(() => {
    // adds letter by letter to textEl. i tracks which letter is being added.
    textEl.textContent += text[i];
    // moves to next letter.
    i++;

    // checks is all letters are added. 
    if (i === text.length) {
      // stops timer.
      clearInterval(interval);
    }
    // typing speed; 60 miliseconds.
  }, 60);
}



// call function. index = 0 = first sentence in messages.
showMessage(messages[index]);

// move forward on Enter
inputEl.addEventListener("keydown", (e) => {
  // only move on enter key.
  ...
  // check if there are sentences left.
  if (index < messages.length) {
    // if less than full, keep showing.
    showMessage(messages[index]);

    // else = if no more sentences, 
  } else {
    // show done.
